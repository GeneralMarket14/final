...
use App\Http\Resources\V2\UserMiniResource;
...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if(!$user){
            $this->failed(translate("User not found"));
        }
        if (empty($request->all())) {
    // The request is empty
   return $this->failed(translate("You have to have value in one of the fields"));
}
    ...
    if ($request->hasFile('avatar')) {
      ...
      return $this->failed(translate('The provided password does not match your current password'));
}
  ...
  //Hash and update the new password
     $user->password = Hash::make($request->new_password);
      $user->country = $request->country ?? $user->country;
        ...
        $user->lga = $request->lga ?? $user->lga;
        $user->cacnumber = $request->cacnumber ??$user->cacnumber;
        $user->cacname = $request->cacname ?? $user->cacname;
        $user->areaofspecialization = $request->areaofspecialization ?? $user->areaofspecialization;
        $user->mainoffice = $request->mainoffice ?? $user->mainoffice;
        $user->tinnumber = $request->tinnumber ?? $user->tinnumber;
         $user->email = $request->email ?? $user->email;
        //  $user->uid = $createdUser ->uid ?? $user->uid; 
    ...
    //     }
        ...
        return new UserMiniResource($user1);
    }
